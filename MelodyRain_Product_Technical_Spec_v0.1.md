...
2. 与预览一致的合成音乐；
3. 1080×1920、30 fps、带音频的 MP4 动画；
...
- 支持竖屏 9:16，生成 1080×1920、30 fps、H.264/AAC MP4；
...
MVP-1 使用连续的“三阶段纵向谱面相机”。谱面、静态蒙版、动态蒙版与所有雨滴共用同一个纵向 transform；相机位置只能由当前绝对 Transport 时间对应的乐谱位置计算，不依赖上一帧积分。

- 当当前落点尚未到达 view 高度的 1/2 时，相机偏移为 0，谱面正常静止显示；
- 当当前演奏谱表行第一次进入 view 高度的 1/2 附近时，进入居中跟随段；同一谱表行演奏期间相机保持稳定，换行时使用可由绝对时间重建的短距离平滑过渡，使当前演奏行维持在画面纵向中央附近；
- 当整首谱面的结尾已经完整显示、相机到达最大偏移时提前停止；停止时当前演奏行仍在画面纵向中央附近，此后谱面保持静止，当前演奏位置再逐步接近画面底部，剩余音符继续下落、命中并停留直到曲终；
...
- 雨滴颜色由 FR-09.5 的全局演奏元素效果决定，不再仅按高低音谱表分配固定颜色；
...
- 所有书面休止符均生成独立的 `RestVisualGroup`，以其 MusicXML 书面起点映射到主 Transport 的时间作为 `hitTimeMs`，使用与音符相同的 `rainDrop`、演奏元素效果、Glow、命中和停留规则；休止符不依赖 MIDI note-on 事件，但必须使用同一 tempo map 换算时间；
...
> 渐强/渐弱发夹不属于本节的跨音符连接符，不执行路径生长；其行为由 FR-09.3 定义。
...
当主 Transport 到达 MusicXML/规范化乐谱模型记录的对应时间位置时，元素在原谱位置执行 `opacity: 0 → 1` 的 300 ms 渐显；不得下落、移动、缩放或沿路径生长。渐显完成后保持可见，直到所属谱面行滚出视口。渐强/渐弱发夹在起始位置到达时以完整几何形状整体淡入，不采用 FR-09.2 的路径生长、裁切或“从左向右展开”规则。踏板线、八度线和 Volta 同样在起始位置到达时整段渐显。跳转或倒退后，当前时间之后的元素必须重新隐藏；当前时间之前且已超过 300 ms 渐显区间的元素必须立即恢复为完全可见。
...
- 蒙版开口支持 `0%` 至 `100%` 的黑色混入比例；实现为背景源上方、同一蒙版内部的普通黑色透明叠加，默认 `40%`，不得使用结果依赖底层画面的 `mix-blend-mode`；
- 浅色谱纸是背景源之上的独立表面层，支持 `0%` 至 `100%` 的透明度；默认 `10%`，透明度提高时在非蒙版区域显示同一份固定背景源，蒙版开口和黑色混入仍绘制在谱纸上方；
...
FR-09.1 至 FR-09.3 管理的全部下落、命中、停留、渐显和节奏生长元素使用同一项全局演奏效果配置，并可在播放前或播放中即时切换：

- `mask` 为默认模式。动态元素使用独立 Alpha Mask 显示与 FR-09.4 相同的背景源和固定 `cover` 裁切位置；在背景源上方以普通透明叠加混入独立颜色，默认颜色 `#1CAEE8`、强度 `50%`，颜色和强度均可修改；不得复用静态谱面的黑色混入值，也不得使用 `mix-blend-mode`；
- `rainbow` 模式按 MusicXML/OSMD 的书写音名字母着色，而不是按 MIDI 半音推断。C、D、E、F、G、A、B 依次使用 `#F05D6C`、`#F39A4A`、`#E9C84A`、`#42C99A`、`#1CAEE8`、`#4F6FEF`、`#8357DF`；升降号和八度不改变字母所属颜色；
- 和弦的每个音头使用自己的音名颜色。符杆向上时，共享符杆使用最低音颜色；符杆向下时，共享符杆使用最高音颜色；附属于单个音头的升降号、附点和加线跟随该音头，共享装饰跟随符杆颜色；
- 八分、十六分及更短时值的连梁按各符杆横坐标设置渐变色标，每根符杆的色标颜色遵循前述和弦规则；次级短连梁只使用其覆盖区间的渐变，不重新从 C 色开始；
- 休止符、速度文字、歌词、排练记号等无明确音高归属的定时元素使用完整 C→B 彩虹渐变；有明确起止音高的延音线、圆滑线和滑音线从起始音颜色渐变到结束音颜色；
- 两种模式都保留透明度、裁切进度、下落 transform 和生命周期。`rainbow` 保留按当前彩虹颜色产生的命中脉冲与 Glow；`mask` 不得继承旧的高低谱表蓝色/紫色滤镜，只显示共享背景源与当前独立混色。模式切换不得重新排版谱面、跳转 Transport 或改变命中坐标；
- 动态蒙版几何只在谱面排版完成后创建一次。播放帧只同步视口附近元素的位移、显隐、透明度和裁切状态；相同彩虹色标必须在同一 SVG 内复用渐变定义，避免长谱初始化或播放卡顿。

### FR-10 MVP-1 全局视觉效果

MVP-1 支持 `rainDrop + glow + performanceMask/rainbowColor`，整首乐曲使用同一套全局配置。暂不支持逐小节、逐次反复、逐声部或单音符覆盖。
...
|---|---|---|
| `performanceMask` | 动态元素共用谱面背景并独立混色 | `mixColor`, `mixAmount`, `source: "scoreMask"` |
| `rainbowColor` | 按 CDEFGAB 着色并为连梁及无音高元素生成渐变 | `palette`, `stemOwner`, `gradient` |
...
演奏元素默认使用 `performanceMask`；用户切换到 `rainbowColor` 后按 FR-09.5 的书写音名、和弦符杆和连梁渐变规则着色。
...
| 属性 | MVP-1 默认值 |
|---|---|
| 分辨率 | 普通：540×960（默认）；高清：1080×1920 |
| 画幅 | 9:16 |
| 帧率 | 30 fps 恒定帧率 |
| 视频编码 | H.264；普通 `veryfast`/CRF 22，高清 `medium`/CRF 18 |
| 像素格式 | yuv420p |
| 音频 | AAC，48 kHz，立体声 |
| 容器 | MP4 |

渲染时刻必须由 `frameIndex / fps` 计算。任何一帧的动画状态都能直接由绝对时间求得，禁止依靠上一帧积分，以保证断点渲染和重复渲染一致。当前默认导出全曲，输出文件名取当前画面标题并追加 `.mp4`；Windows 非法文件名字符必须替换为安全字符。

导出任务由本地 Express 服务管理。Playwright Core 驱动本机 Chrome 或 Edge 打开专用导出页面，按帧请求同一个 React/OSMD 舞台重建状态，并把舞台截图以 PNG image pipe 直接写入 FFmpeg；默认不在磁盘保留完整 PNG 序列。FFmpeg 使用 H.264/AAC、`yuv420p` 和 fast-start MP4 封装素材 MP3。

导出完成后的保存策略：

- 素材目录句柄具有 `readwrite` 权限时，使用 File System Access API 直接写入素材文件夹；
- 无目录句柄、权限被拒绝或写入失败时，回退为浏览器下载；
- 保存完成后必须弹窗通知用户文件名以及保存到素材文件夹或下载目录；
- 导出任务支持轮询进度、取消和临时文件清理。
...
- 在 30 fps 导出中，动画命中帧与音频 note-on 的偏差绝对值 ≤ 1 帧（33.33 ms）；
- P95 偏差 ≤ 1 帧，最大偏差 ≤ 2 帧；
- 最终音视频轨总时长差 ≤ 20 ms；
...
- 3 分钟钢琴谱解析与预览生成 ≤ 60 秒；
- 30 秒 1080×1920、30 FPS 视频的软件渲染目标 ≤ 10 分钟；
...
- 渐强/渐弱发夹必须以完整形状执行 300 ms 整体淡入，不得表现为从左向右生长；
- 预览与离线导出在同一绝对时间的透明度，以及 FR-09.2 连接符（不含发夹）的生长比例必须一致。
...
- 当前演奏位置到达画面纵向 1/2 前，谱面偏移保持为 0；
- 当前演奏谱表行进入纵向居中区域后，谱面跟随当前谱表行；同一行演奏期间保持稳定，换行时平滑移动到新的居中位置，不得突然跳动；
- 谱面到达最大偏移后停止，最后阶段的音符继续下落直到曲终；
...
- 生成 1080×1920、9:16、30 fps 竖屏 MP4；
...
当前已确认：MVP-1 是本地网页应用；用户手工选择本地 MusicXML/MIDI/MP3；输出固定为 9:16、30 FPS 竖屏视频，提供 540×960 普通档与 1080×1920 高清档，不开发横屏方案；乐谱符号按原尺寸的 2/3 等比例显示，竖屏默认每行 2 小节并实时预览；效果包括音符下落、命中、停留、共享蒙版与彩虹模式；纵向相机在当前谱表行到达画面中线前静止，随后居中跟随当前行，并在谱尾完整显示时提前停止；预览显示绝对视频帧号，导出默认覆盖全曲，也允许选择包含起始帧、不包含结束帧的局部范围；本地服务使用 Playwright 驱动 Chrome/Edge 逐帧渲染并调用 FFmpeg；成品优先保存到素材文件夹，失败时回退到浏览器下载。
...
- OSMD 将 MusicXML 渲染为 SVG；当前支持每行 1 至 6 小节、固定 9:16 竖屏 profile，以及“中线前静止—居中跟随当前谱表行—谱尾完整显示后停止”的三阶段纵向谱面相机。
- MIDI tempo map、MP3 播放、进度跳转、0.9×/0.95×/1× 变速、1.2 秒动画预卷由单个 `MediaTransport` 驱动。
...
设置文件名固定为 `melody-rain.settings.json`，当前开发格式版本为 `5`。保存与读取只接受当前版本；项目尚未发布，因此不维护旧版设置迁移。`noteFrameEffect` 集中保存共享混入强度和帧范围。单色范围复用原始 `performanceMask` 语义；彩虹范围不使用混入强度。范围结束或范围之间存在空档时，最近一个范围持续生效，直到下一范围接管或视频结束。所有范围边界都按 60 帧过渡在绝对帧号上平滑过渡：同色单色边界直接插值颜色，单色与彩虹之间的模式切换经彩虹渲染器从上一样式调色板逐色插值到当前样式（单色等价为全同色调色板），颜色强度同时从共享混入强度插值到全强度（反向切换亦然），过渡结束后才落到最终渲染器，端点保持精确。`sample/ode-to-joy/melody-rain.settings.json` 是默认配置参考。
...
- 当前只支持全曲导出，尚未实现按起止小节选择导出区间、任务恢复、持久化队列或 `quality-report.json`。
...
- 没有将任务状态、解析中间产物或渲染产物持久化；导出任务位于内存，临时素材与成品位于系统临时目录，IndexedDB 当前只保存最近目录句柄。
...
- 截至本章日期，仓库测试基线为 18 个测试文件、79 项测试；类型检查和生产构建应作为每次发布的最低门禁。
...
- `src/components/ui/`：共享展示原语（按钮、分段选择、滑杆、颜色字段、键值列表、卡片标题、状态 pill）；展示组件一律复用原语，不内联拼装样式类，设计令牌集中在 `src/styles/_tokens.scss`。

OSMD 固定为 `2.0.0`。所有依赖 OSMD/VexFlow SVG DOM 的选择器和结构类型必须通过 `osmd-compat.ts` 暴露；升级 OSMD 前必须先通过真实 MusicXML 集成回归。

### 15.6 未来迁移到云端的视频导出注意事项

本节只记录未来迁移约束，不表示云端导出已经实现。当前版本继续使用本地单任务、全曲连续逐帧渲染，不采用本地 30 秒分片方案。迁移前必须保留“绝对时间决定每帧状态”的现有语义，云端与本地相同输入在同一帧号上应得到相同的谱面位置和动画状态。

#### 15.6.1 初始机器规格

- 推荐从 Linux `8 vCPU / 16 GB RAM / 50 GB SSD` 的专用或计算优化实例开始压测；普通 540×960 单任务最低可用 `4 vCPU / 8 GB RAM`，但不保证快于当前本地电脑；
- 高清 1080×1920 或 2–3 个并行任务建议 `16 vCPU / 32 GB RAM / 100 GB SSD`；实际并发上限必须通过真实长曲目压测决定；
- 当前瓶颈主要是 Chrome 逐帧页面截图、PNG 编码/传输和 FFmpeg `libx264` 软件编码，首版无需 GPU。只有改用 WebGL 重特效或 FFmpeg 硬件编码后，才评估 GPU 实例；
- 临时目录必须位于本地 SSD，并监控空间、inode、内存、swap、CPU 持续频率和任务耗时。磁盘容量需要覆盖上传素材、临时文件和未领取成品的峰值，而不能只按最终 MP4 大小估算。

#### 15.6.2 Linux 与运行时兼容

- 建议使用 Ubuntu LTS、受支持的 Node.js LTS、固定版本的 FFmpeg 和 Playwright Chromium，并把这些版本写入容器镜像或部署清单；
- 当前 `chromeExecutable()` 包含 Windows Chrome/Edge 固定路径。迁移时必须支持 `MELODY_RAIN_CHROME`、Linux Chromium 路径，或改用 Playwright 管理的浏览器可执行文件；启动时执行浏览器和 FFmpeg 健康检查；
- 安装 Playwright/Chromium 所需的 Linux 动态库，并在镜像中安装中文字体、拉丁字体和乐谱相关字体。发布前对字体回退、文字宽度、OSMD 换行和 SVG 截图做视觉回归；
- 云端不能依赖浏览器 File System Access API 把成品写回用户素材文件夹。成品应保存到对象存储，并通过有时效的签名下载地址交付；
- 保持 FFmpeg 参数数组调用，禁止把标题、文件名、路径或其他用户输入拼接成 shell 命令。

#### 15.6.3 服务与任务架构

- Web/API 服务只负责鉴权、上传、创建任务、查询状态和签发下载地址；耗时渲染放入独立 worker，不能占用 API 进程；
- 使用持久化队列和数据库保存任务状态、输入对象键、配置版本、代码版本、浏览器/FFmpeg 版本、进度、错误和输出对象键。服务或 worker 重启后任务必须可诊断，不能继续只依赖进程内 `Map`；
- 每个 worker 设置严格并发上限。一个任务拥有独立浏览器 context、临时目录和 FFmpeg 子进程；任务结束、取消、超时或异常时都必须关闭浏览器、终止子进程并清理事件监听器；
- 先实现单任务连续渲染的云端版本。只有长任务失败恢复或吞吐量成为明确问题后，再考虑云端分片与并行；分片时必须按绝对帧区间渲染，防止边界重复/漏帧，并优先拼接无音频视频后统一封装完整音轨；
- 设置最大曲目时长、最大总帧数、单帧超时、任务总超时、重试次数和取消宽限期。重试必须具有幂等任务键，避免产生多个收费任务或重复成品；
- 进度查询初期可继续轮询，规模扩大后可改为 SSE/WebSocket；状态更新频率需要节流，避免每帧写数据库。

#### 15.6.4 上传、隔离与安全

- 对 MusicXML/MXL、MIDI、MP3、背景图片和设置文件分别限制扩展名、MIME、单文件大小、文件数量及项目总大小；服务端重新执行现有 MXL/XML 安全校验，不能信任前端结果；
- 上传文件使用服务生成的对象键，不直接采用用户文件名作为路径。对显示文件名做规范化，阻止路径穿越、控制字符和覆盖同名对象；
- 渲染页面只允许访问本任务的素材和受控静态资源，设置 CSP 和网络访问限制，阻止任意外部 URL、SSRF、内网探测及跨任务资产读取；
- worker 使用非 root 用户、只读应用文件系统、受限临时目录和资源配额运行。不同用户/任务之间不得共享可写浏览器 profile；
- 为创建任务、状态查询、取消和下载接口增加鉴权、授权、速率限制和审计日志。日志不得记录原始乐谱、音频内容、签名下载 URL 或其他敏感数据；
- 明确素材与成品的保留期、用户删除能力、备份策略和隐私说明。临时文件应在任务结束后尽快删除，对象存储通过生命周期规则自动过期。

#### 15.6.5 性能、可靠性与验收

- 上云前建立本地基准集：普通/高清、30 秒/3 分钟/长曲目、简单/复杂乐谱各至少一个样本，记录总耗时、平均帧耗时、峰值内存、CPU 和输出大小；
- 云端压测至少覆盖单任务、目标并发、取消、浏览器崩溃、FFmpeg 失败、磁盘不足、worker 重启、上传中断和对象存储失败；
- 监控队列等待时间、任务成功率、每帧耗时、Chrome/FFmpeg RSS、CPU 时间、临时磁盘、输出大小和每分钟视频成本，并按质量档位分别统计；
- 使用固定素材比较本地与云端输出：总帧数一致、首尾帧无缺失、动画命中误差满足 NFR-01、音视频总时长差满足既有指标、字体与排版无变化；
- 首版默认只运行一个导出任务，测得稳定资源峰值后再提高并发。云端机器 vCPU 数量不能直接等同于本地 CPU 线程数，应以持续性能和真实导出耗时选型；
- 对象存储上传成功且校验输出文件可读后才将任务标记为 `completed`。下载地址生成失败不应重新渲染视频，应允许重新签发。

#### 15.6.6 推荐迁移顺序

1. 把 Chrome/FFmpeg 路径、端口、临时目录和输出存储抽象为配置；
2. 建立可复现的 Linux 容器镜像，并通过固定素材完成视觉与音画同步回归；
3. 把上传和成品改为对象存储，把进程内任务状态改为持久化队列/数据库；
4. 将 API 与渲染 worker 分离，补齐超时、取消、重试、清理和可观测性；
5. 以 `8 vCPU / 16 GB RAM` 单并发开始基准测试，再根据成本和耗时决定是否升级到 `16 vCPU / 32 GB RAM` 或增加 worker；
6. 在安全、隐私、故障恢复和输出一致性验收通过后，再开放给外部用户。
