...
当前已确认：MVP-1 是本地网页应用；用户手工选择本地 MusicXML/MIDI/MP3；只输出 1080×1920、9:16、30 FPS 竖屏视频，不开发横屏方案；乐谱符号按原尺寸的 2/3 等比例显示，竖屏默认每行 2 小节并实时预览；效果包括音符下落、命中、停留、共享蒙版与彩虹模式；纵向相机在当前谱表行到达画面中线前静止，随后居中跟随当前行，并在谱尾完整显示时提前停止；当前导出范围为全曲；本地服务使用 Playwright 驱动 Chrome/Edge 逐帧渲染并调用 FFmpeg；成品优先保存到素材文件夹，失败时回退到浏览器下载。
