...
**MVP-1 目标：** 在本地网页应用中导入用户预先下载的 MusicXML 与 MIDI，生成可播放五线谱和带同步音乐的横屏动画视频。

---

## 1. 产品定义

MelodyRain 将 MusicXML 与 MIDI 转换为可播放的数字五线谱，并进一步生成“音符从上方落下，在奏响时停留于原谱对应音符位置，同时发光、变色”的动画视频。MVP-1 输出横屏视频；竖屏及其他画幅在 MVP-2 中实现。
...
- 支持横屏 16:9，生成 1920×1080、60 fps、H.264/AAC MP4；
...
1. 用户在 MuseScore 手工下载同一乐谱的 MusicXML/MXL 与 MIDI；
2. 用户打开本地网页应用并选择两个文件；
3. 系统在本地校验文件格式和内容一致性；
...
- 用户分别选择 MusicXML/MXL 与 MIDI 文件；
- 两个文件必须来自同一份乐谱和同一导出版本；
- 支持 `.musicxml`、`.xml`、`.mxl`、`.mid`、`.midi`；
- PDF、音频和 MSCZ 不属于 MVP 必需资源；
...
MVP-1 使用“纵向多系统舞台”：当前演奏行固定在主要落点区域，屏幕同时可见上一行、当前行和下一行的全部或部分内容。当前行播放完后，谱面整体纵向向上滑动，使下一行进入主要落点区域。

- 默认在当前行最后一个新发音完成后开始纵向滑动；
- 若存在跨系统延音，必须等待延音结束后再滑动；
- 纵向滑动期间允许下一行音符已经开始下落；
- 谱面和正在下落的音符使用同一世界坐标与 camera transform，因此滑动不会破坏雨滴与目标 notehead 的相对关系；
- 纵向滑动使用可配置时长，MVP-1 默认 300 ms；
...
画幅仍由 `RenderProfile` 定义以保留扩展性，但 MVP-1 只启用一个横屏 profile。竖屏、多 profile 同时导出属于 MVP-2。
...
|---|---|---:|---:|
| `landscape-16x9` | 横屏 16:9 | 1920×1080 | 4 |
...
- 横屏 RenderProfile 单独执行 MusicXML 重排版；
...
- MVP-2 增加竖屏时，竖屏不得由横屏裁切得到，必须重新排谱、重新计算坐标并重新渲染。
...
- 原谱中需要动画的音符头不直接显示，雨滴是其动画视觉替代；谱号、符干、横梁、连线等其他记谱元素正常显示；
...
- 首批音符若无足够预卷时间，视频自动增加最多 1.5 秒静音预卷；
- 休止符不生成下落音符；
...
渲染时刻必须由 `frameIndex / fps` 计算。任何一帧的动画状态都能直接由绝对时间求得，禁止依靠上一帧积分，以保证断点渲染和重复渲染一致。MVP-1 输出文件名为 `melody-rain.landscape.mp4`。
...
2. SVG 是相对静态的谱面底图；原始 notehead 的隐藏、雨滴、发光和选区高亮由单独叠层完成；
...
`TempoSegment` 必须覆盖常速、离散 tempo 标记、渐快/渐慢及显式停顿；不得只保存单个“全曲 BPM”。`TransportSnapshot` 是一帧内所有消费者共享的不可变快照。消费者不得在读取快照后自行累加时间。
...
左右手同时演奏、包含加线音符时，动画不得落入错误谱表或相邻音高位置。
...
给定 MVP-1 横屏 RenderProfile：

- 生成 1920×1080、60 fps 横屏 MP4；
...
当前已确认：MVP-1 是本地网页应用；用户手工选择本地 MusicXML/MIDI；只输出横屏 16:9；横屏默认每行 4 小节并实时预览；效果为整曲全局 `rainDrop + glow + colorize`；颜色跟随谱表；雨滴命中后停留到所属行滚出视口；和弦使用覆盖全部音头的多音头组合雨滴；谱面行播放完后纵向向上滑动；超长小节横向滚动时演奏位置固定在画面中央；用户选择导出起止小节；本地服务调用本机 FFmpeg；MVP-2 再实现竖屏、更多动画、增强音轨和自动下载。
