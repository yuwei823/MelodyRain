...
当前已确认：MVP-1 是本地网页应用；用户手工选择本地 MusicXML/MIDI；只输出 1080×1920、9:16 竖屏视频，不开发横屏方案；乐谱符号按原尺寸的 2/3 等比例显示，竖屏默认每行 3 小节并实时预览；效果为整曲全局 `rainDrop + glow + colorize`；颜色跟随谱表；雨滴命中后停留到所属行滚出视口；和弦使用覆盖全部音头的多音头组合雨滴；谱面行播放完后纵向向上滑动；超长小节横向滚动时演奏位置固定在画面中央；用户选择导出起止小节；本地服务调用本机 FFmpeg；MVP-2 再实现更多动画、增强音轨、平台专用竖屏 profile 和自动下载。
