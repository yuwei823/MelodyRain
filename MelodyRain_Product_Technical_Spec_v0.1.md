...
MVP-1 使用连续的“三阶段纵向谱面相机”。谱面、静态蒙版、动态蒙版与所有雨滴共用同一个纵向 transform；相机位置只能由当前绝对 Transport 时间计算，不依赖上一帧积分。

- 当当前落点尚未到达 view 高度的 1/2 时，相机偏移为 0，谱面正常静止显示；
- 当演奏进度第一次到达或越过 view 高度的 1/2 所对应的书面音符位置时，进入匀速段；相机偏移在启动时刻与结束时刻之间按线性函数连续增加，不使用 CSS easing；
- 匀速段在最后一个音符开始执行 1.2 秒下落动画时到达最大偏移并停止；此后谱面保持静止，剩余音符继续下落、命中并停留直到曲终；
...
- 到达纵向 1/2 后，谱面以恒定像素速度连续向上移动，不得出现按系统跳动或缓动滞后；
...
当前已确认：MVP-1 是本地网页应用；用户手工选择本地 MusicXML/MIDI/MP3；只输出 1080×1920、9:16、30 FPS 竖屏视频，不开发横屏方案；乐谱符号按原尺寸的 2/3 等比例显示，竖屏默认每行 2 小节并实时预览；效果包括音符下落、命中、停留、共享蒙版与彩虹模式；纵向相机在演奏位置到达画面中线前静止，随后匀速滚动，并在最后音符下落阶段开始前到达底部停止；当前导出范围为全曲；本地服务使用 Playwright 驱动 Chrome/Edge 逐帧渲染并调用 FFmpeg；成品优先保存到素材文件夹，失败时回退到浏览器下载。
...
- OSMD 将 MusicXML 渲染为 SVG；当前支持每行 1 至 6 小节、固定 9:16 竖屏 profile，以及“中线前静止—匀速滚动—底部停止”的三阶段纵向谱面相机。
