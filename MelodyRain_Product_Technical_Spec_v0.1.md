...
> 渐强/渐弱发夹同样按起止节拍从左向右生长，但必须使用原生 SVG 描边动画实现：将组成发夹的线段归一化为 `pathLength="1"`，通过 `stroke-dasharray` 和 `stroke-dashoffset` 显示当前进度。发夹不得使用矩形 `clip-path` 模拟生长；渐弱发夹也必须保持从画面左侧向右侧展开。
...
当主 Transport 到达 MusicXML/规范化乐谱模型记录的对应时间位置时，元素在原谱位置执行 `opacity: 0 → 1` 的 300 ms 渐显；不得下落、移动或缩放。渐显完成后保持可见，直到所属谱面行滚出视口。渐强/渐弱发夹是例外，由 FR-09.2 的原生 SVG 描边规则控制，不属于本节的整体淡入元素。踏板线、八度线和 Volta 在起始位置到达时整段渐显。跳转或倒退后，当前时间之后的元素必须重新隐藏；当前时间之前且已超过 300 ms 渐显区间的元素必须立即恢复为完全可见。
...
- 渐强/渐弱发夹必须通过原生 SVG `stroke-dasharray` / `stroke-dashoffset` 从左向右生长，渐强和渐弱方向均不得反转，也不得退化为 300 ms 整体淡入或矩形裁切；
- 预览与离线导出在同一绝对时间的透明度和 FR-09.2 连接符生长比例必须一致。
