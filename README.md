...
- 通过本地 Chrome 逐帧渲染，并由 FFmpeg 导出 `1080 × 1920`、30 FPS、H.264/AAC MP4 视频，支持进度显示与取消；
- 音符、和弦、休止符及相关记谱元素随演奏下落、命中并停留；
- 谱面采用三阶段纵向相机：演奏位置到达画面中线前静止，随后匀速上移，到达谱面底部后停止；
...
- 固定输出 `1080 × 1920`、9:16、30 FPS MP4；
- 导出范围目前为全曲，包含 1.2 秒音符入场预卷；
- 支持任务进度、取消和错误提示；
