...
- 演奏元素模式、混入颜色和混入强度。
- 共享连杆音符模式。
- 帧范围着色的 15 帧默认过渡长度及自定义范围。

“NOTE FRAME EFFECT / 音符帧效果”统一管理共享混入强度和帧范围着色。首次载入时自动建立从第 1 帧到最后一帧的 `Range 1`，可选择单色或彩虹色。单色恢复为共用谱面背景源并按混入强度叠加颜色：强度降低时回到背景图片或背景色，而不是固定趋近黑色。彩虹色不使用该强度并始终显示完整色板。范围结束或范围之间存在空档时，沿用最近一个范围的设置，直到下一个范围接管或视频结束。同模式的颜色边界按绝对帧号过渡；单色和彩虹色之间直接切换渲染模式。

文件名固定为 `melody-rain.settings.json`。浏览器具有素材文件夹写入权限时会直接保存到该文件夹；否则浏览器会下载文件，需要手动将它放回素材文件夹。点击“读取参数”可重新应用已发现的参数文件，或另行选择 JSON 文件。

当前开发格式为 v5，只读取当前版本。完整且可编辑的默认配置可参考 `sample/ode-to-joy/melody-rain.settings.json`；其中包含可直接读取的 `noteFrameEffect` 混入强度、过渡帧数和至少一个从第 1 帧开始的范围。
