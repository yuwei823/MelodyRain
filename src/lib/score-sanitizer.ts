...
const OCTAVE_SHIFT_ELEMENT_PATTERN = /<(?:(?:[A-Za-z_][\w.-]*):)?octave-shift\b[^>]*(?:\/>|>[\s\S]*?<\/(?:(?:[A-Za-z_][\w.-]*):)?octave-shift\s*>)/gi;
...
/** Removes printed 8va/8vb octave-shift lines without changing note pitches or timing. */
export function removeOctaveShiftMarkings(musicXml: string): string {
  return musicXml
    .replace(OCTAVE_SHIFT_ELEMENT_PATTERN, "")
    .replace(EMPTY_DIRECTION_TYPE_PATTERN, "")
    .replace(EMPTY_DIRECTION_PATTERN, "");
}

export function sanitizeScoreMusicXml(musicXml: string): string {
  return removeTupletMarkings(removeOctaveShiftMarkings(removePedalMarkings(musicXml)));
}
